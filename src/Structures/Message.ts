    ...
    constructor(private M: proto.IWebMessageInfo, private client: Client) {
        ...
        const getContent = (): string => {
            ...
            if (M.message?.interactiveResponseMessage)
                return (
                    JSON.parse(M.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson as string)?.id || ''
                )
        ...
        if (M.message?.[this.type as 'extendedTextMessage']?.contextInfo?.quotedMessage) {
            ...
            if (quotedMessage && participant && stanzaId) {
                ...
                const getQuotedContent = (): string => {
                    ...
                    if (quotedMessage?.interactiveResponseMessage)
                        return (
                            JSON.parse(quotedMessage?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson as string)
                                ?.id || ''
                        )
