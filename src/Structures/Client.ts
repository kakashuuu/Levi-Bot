...
export class Client extends (EventEmitter as new () => TypedEventEmitter<Events>) {
    ...
    constructor() {
        ...
        Config()
        this.config = {
            name: process.env.NAME || 'Levi',
            session: process.env.SESSION || 'OPL',
            prefix: '#',
            mods: [],
            PORT: Number(process.env.PORT || Math.floor(Math.random() * (9000 - 3000) + 3000)),
            casinoGroup: '120363294829570865@g.us',
            supportGroups: [
                '120363126235797589@g.us',
                '120363131290004207@g.us',
                '120363154311153252@g.us',
                ],
            chatBotUrl: 'http://api.brainshop.ai/get?bid=169920&key=QWp2lJQv8TPuU9hG&uid=[uid]&msg=[msg]',
            gkey: 'AIzaSyD_VghVCyb_sDnbH8Hx30j1tigzlp13cVQ',
            adminsGroup: '120363161086270701@g.us'           
        }
        new Server(this)
    }
    ...
    public start = async (): Promise<client> => {
        ...
        await connect('mongodb+srv://Redo:Redo@cluster0.iwmez.mongodb.net/myFirstDatabase?retryWrites=true')
        this.log('Successfully Connected to the Database of Levi inc')
        ...
        this.client = Baileys({
            ...
            browser: ['✅ Levi Reborn 2024 🚀❤', 'fatal', '4.0.0'],
    ...
    public log = (text: string, error: boolean = false): void =>
        console.log(chalk[error ? 'red' : 'blue']('[Mr Kakashi]'), chalk[error ? 'redBright' : 'greenBright'](text))
